...
namespace ModulYonetici
{
    public partial class BilgisayarAdlari : BasePage
    {
        ...
        protected void btnExcelYukle_Click(object sender, EventArgs e)
        {
            Page.ClientScript.RegisterStartupScript(this.GetType(), "TriggerFileUpload", "TriggerFileUpload();", true);
            fuExcel.Attributes["onchange"] = "document.getElementById('" + Page.Form.ClientID + "').submit();";
        }
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            if (fuExcel.HasFile && fuExcel.PostedFile != null && fuExcel.PostedFile.ContentLength > 0)
            {
                //ExceldenYukle();
            }
        }
        ...
        //    try
        //    {
        //        string dosyaYolu = Server.MapPath("~/Temp/") + Guid.NewGuid() + Path.GetExtension(fuExcel.FileName);

        //        if (!Directory.Exists(Server.MapPath("~/Temp/")))
        //            Directory.CreateDirectory(Server.MapPath("~/Temp/"));

        //        fuExcel.SaveAs(dosyaYolu);
        ...
        //            {
        //                string domainNo = worksheet.Cells[row, 1].Text?.Trim().ToUpper();
        //                string kisiAdi = worksheet.Cells[row, 2].Text?.Trim();
        //                string bilgisayarTipi = worksheet.Cells[row, 3].Text?.Trim();
        //                string dahiliNo = worksheet.Cells[row, 4].Text?.Trim();
        ...
        //                string checkQuery = "SELECT COUNT(*) FROM bilgisayar_adlari WHERE domain_no = @DomainNo";
        //                var checkParams = CreateParameters(("@DomainNo", domainNo));
        //                int varMi = Convert.ToInt32(ExecuteScalar(checkQuery, checkParams));

        //                if (varMi > 0)
        //                {
        //                    string updateQuery = @"UPDATE bilgisayar_adlari 
        //                                          SET kisi_adi = @KisiAdi, 
        //                                              bilgisayar_tipi = @BilgisayarTipi, 
        //                                              dahili_no = @DahiliNo,
        //                                              guncelleme_tarihi = @GuncellemeTarihi,
        //                                              guncelleyen_kullanici = @GuncellemeKullanici
        //                                          WHERE domain_no = @DomainNo";
        ...
        //            ShowToast($"Excel yükleme tamamlandı! {eklenenSayisi} yeni kayıt, {guncellenenSayisi} güncelleme.", "success");
        //            KayitlariListele();
        //        }
        //    }
        //    catch (Exception ex)
        //    {
        //        LogError("ExceldenYukle hatası", ex);
        //        ShowToast("Excel yükleme sırasında hata oluştu!", "error");
        //    }
        //}
